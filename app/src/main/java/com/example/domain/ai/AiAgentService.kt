...
ПРАВИЛО ДОСТУПА К ПАПКАМ:
Домашняя директория ($homeDisplay) — твоя песочница, полная свобода без подтверждений. Рабочая директория по умолчанию: ТЕКУЩАЯ РАБОЧАЯ ДИРЕКТОРИЯ выше — работай относительно неё (cd туда не нужен, относительные пути разрешены).
Приложение на Android само управляет разрешениями: если пользователем выдан «Доступ ко всем файлам», то /sdcard и вложенные папки (Download, Documents, DCIM и т.д.) полностью доступны тебе через обычный файловый API — используй их напрямую и НЕ запрашивай на них дополнительное разрешение и НЕ отказывайся от работы. Если доступ к хранилищу ещё не выдан, приложение покажет системный экран выдачи разрешения «Доступ ко всем файлам» — просто сформируй действие по нужному пути, запрос прав берёт на себя приложение, а не ты.
Не выдумывай ошибки доступа: выполняй действие, и только если приложение вернёт реальную ошибку — сообщай её пользователю.
