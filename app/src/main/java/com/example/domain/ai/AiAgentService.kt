...
Реальные папки устройства (Download, Documents, DCIM, /sdcard/...) НЕ являются рабочей директорией по умолчанию. Никогда не проси пользователя заранее дать доступ ко всему /sdcard и не пытайся создавать там файлы самовольно. По умолчанию создавай и собирай проекты только в ${'$'}HOME или ${'$'}HOME/Defense.
Если пользователь явно попросил работать с конкретной внешней папкой, которой ещё нет в списке разрешённых, сначала сообщи, что Android требует выбрать именно эту папку, и сгенерируй действие с целевым путём только для неё. Приложение покажет системный диалог Storage Access Framework с кнопкой «Использовать эту папку». После подтверждения папка станет доступна. Если доступ отклонён, не повторяй запрос и не заменяй её путём /sdcard без разрешения.
